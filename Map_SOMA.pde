...
void setup() {
  size(700, 700);
  mapImage = loadImage("map5.png");
  ohYeah = loadImage("end.png");
  printArray(PFont.list());
  f = createFont("25312.otf", 24);
...
void draw() {
  ...
  image(mapImage, 0, CENTER);

  //text("Mood and Location",100,650);
...
//if the mouse is hovering over circle, show information as text
  if ((d<radius+2) && (d<closestDist)) {
    ...
    //text("CCA Graphic Design",100,100);
    //stroke(#FF0307);
